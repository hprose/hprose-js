...
(function(global){'use strict';global.hprose={};})(this);(function(global){'use strict';if(typeof global.setTimeout!=="undefined"){return;}
...
var timer=deviceone.mm("do_Timer");timer.delay=delay;timer.interval=delay;timer.on('tick',function(){timer.stop();func();});timer.start();return timer;};global.clearTimeout=function(timer){if(timer.isStart()){timer.stop();}};})(this);(function(global){'use strict';if(typeof(global.btoa)==="undefined"){global.btoa=(function(){var base64EncodeChars='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.split('');return function(str){var buf,i,j,len,r,l,c;i=j=0;len=str.length;r=len%3;len=len-r;l=(len/3)<<2;if(r>0){l+=4;}
...
return out.join('');};})();}})(this);(function(global,undefined){'use strict';var propertyToMethod=function(prop){if('get'in prop&&'set'in prop){return function(){if(arguments.length===0){return prop.get();}
...
return m;};}})(this);(function(global){if(typeof global.TimeoutError!=='function'){var TimeoutError=function(message){Error.call(this);this.message=message;this.name=TimeoutError.name;if(typeof Error.captureStackTrace==='function'){Error.captureStackTrace(this,TimeoutError);}}
TimeoutError.prototype=global.hprose.createObject(Error.prototype);TimeoutError.prototype.constructor=TimeoutError;global.TimeoutError=TimeoutError;}})(this);(function(global,undefined){'use strict';if(global.setImmediate){return;}
...
function apiPost(request,env){var future=new Future();global.api.ajax({url:self.uri(),method:'post',data:{body:request},timeout:env.timeout,dataType:'text',headers:_header,certificate:self.certificate},function(ret,err){if(ret){future.resolve(ret);}
...
http.on("success",function(data){future.resolve(data);});http.on("fail",function(result){future.reject(new Error(result.status+":"+result.data));});http.request();return future;}
...
else{global.hprose.filter.JSONRPCClientFilter=global.hprose.JSONRPCClientFilter;}})(this);(function(global){'use strict';global.hprose.common={Completer:global.hprose.Completer,Future:global.hprose.Future,ResultMode:global.hprose.ResultMode};global.hprose.io={StringIO:global.hprose.StringIO,ClassManager:global.hprose.ClassManager,Tags:global.hprose.Tags,RawReader:global.hprose.RawReader,Reader:global.hprose.Reader,Writer:global.hprose.Writer,Formatter:global.hprose.Formatter};global.hprose.client={Client:global.hprose.Client,HttpClient:global.hprose.HttpClient};if(typeof define==='function'){if(define.cmd){define('hprose',[],global.hprose);}
else if(define.amd){define('hprose',[],function(){return global.hprose;});}}
if(typeof module==='object'&&typeof module.exports==='object'){module.exports=global.hprose;}})(this);